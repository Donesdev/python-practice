...
# We are asking the user to enter in a value between 1 and 100 
# and store it in guess

guess = int(input("Guess a number between 1 to 100: "))

# Here is the start to our loop that will keep running as long as the 
# guess is not the same as the randomNumner

while guess != randomNumber: 
    numberOfGuesses += 1 #We increment the numberOfGuesses by 1 each time the loop runs
    if guess == randomNumber:
        break
    if guess > randomNumber: # if the guess is larger than the random #
        print(guess, "is too high.")
        guess = int(input("Guess a lower number: "))
    elif guess < randomNumber: #If the guess is lesser than the random #
        print(guess, "is too low.")
        guess = int(input("Guess a higher number: "))
    else: #If the guess is the same (sinse we've checked both ranges already)
        print("You got it! ")
        break
